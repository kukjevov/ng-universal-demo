...
export class ScriptSvgNode extends SvgNode implements SvgNodeDynamicNode
{
    ...
    {
        const metadata: DynamicComponentRelationMetadataGeneric<ScriptNodeOptions> = await super._getMetadata();

        const compiledScript = await this._injector.get(CodeService).getCompiled(this._codeMetadata)
    ...
    protected _renderVisuals()
    {
        ...
        const currentHeight = this._getHeight();

        const buttonGroup = this._miscGroup.append('g')
            .on('click', () =>
            {
                this._showCode();
            });
