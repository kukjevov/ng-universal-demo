...
export class ComponentLoader
{
    ...
    public async resolveComponentFactory(componentMetadata: DynamicComponentMetadata, parentInjector: Injector): Promise<{factory: ComponentFactory<DynamicComponent>, module: NgModuleRef<any>}>
    {
        ...
        const componentPackageName = `${componentMetadata.componentPackage}-${componentMetadata.componentName}`;
        ...
        if(!npmPackage)
        {
            ...
            {
                const dynamicModule: DynamicModule = await this._moduleLoaders[x].tryLoadModule(componentMetadata);
    ...
    public static async resolveComponentFactory(dynamicModule: DynamicModule, parentInjector: Injector, componentName: string): Promise<{factory: ComponentFactory<DynamicComponent>, module: NgModuleRef<any>}>
    {
        ...
        if(!moduleFactory)
        {
            const componentResolver: ComponentFactoryResolver = parentInjector.get(ComponentFactoryResolver);
        ...
        const componentFactory = moduleRef.componentFactoryResolver.resolveComponentFactory(dynamicModule.component);
