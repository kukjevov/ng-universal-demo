...
export class DynamicComponentPageComponent implements OnInit, OnDestroy
{
    ...
    public ngOnInit()
    {
        this._urlChangeSubscription = this._route.url.subscribe(async urlChanges =>
        {
            ...
            const dynamicContentId = urlChanges[0];
            const tmp = await this._http.get<RemoteDynamicContentMetadata>(`${this._urlPrefix}/${dynamicContentId}`)
            ...
            const metadata: DynamicContentMetadata =
            {
                layout: null,
                relations: []
            };
