...
export class ComponentRelationManager
{
    ...
    private async _initializeRelations(metadata: DynamicComponentRelationMetadata[])
    {
        ...
        {
            const outputs: DynamicComponentRelationManagerInputOutputMetadata[] = [];

            meta.outputs.forEach(output =>
            {
                output.inputs.forEach(input =>
                {
                    const relation = this._relations[input.id];
                    const nodeInstance = relation && relation.nodeInstance ? relation.nodeInstance : null;

                    const inputOutput: DynamicComponentRelationManagerInputOutputMetadata = 
                    {
                        inputId: input.id,
            ...
            if(meta.nodeType)
            {
                const nodeDefinitions: {[name: string]: NodeDefinitionConstructor} = (await import('../../nodeDefinitions')
                    .catch(error =>
                    {
