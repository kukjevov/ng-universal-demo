...
export class DroppableDirective
{
    ...
    {
        const elementRect: ClientRect = this._element.nativeElement.getBoundingClientRect();

        //TODO optimalizovat zbytocne prepocitavanie a nastavovanie css classy ak sa nic nemeni
        switch (this.orientation)
        {
            case 'vertical':
                const mousePosPercent_Y = ((mouseY-elementRect.top) /(elementRect.bottom-elementRect.top))*100;
                ...
                break;
            case 'horizontal':
                const mousePosPercent_X = ((mouseX-elementRect.left) /(elementRect.right-elementRect.left))*100;
