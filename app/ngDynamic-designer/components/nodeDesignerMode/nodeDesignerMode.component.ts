...
export class NodeDesignerModeComponent implements OnDestroy, AfterViewInit
{
    ...
    public ngAfterViewInit()
    {
        ...
        {
            const found = this.nodeComponentPallete.usedComponents.find(itm => itm.component == component);
            const index = this.nodeComponentPallete.usedComponents.indexOf(found);
    ...
    public drop(event: any)//DragEvent
    {
        ...
        {
            const component = this.nodeComponentPallete.availableComponents.find(itm => itm.component.id == event.dataTransfer.getData('text/id'));
        ...
        if(type == NODE_DRAG)
        {
            const node = this.nodeComponentPallete.nodesDefinitions.find(itm => itm.name == event.dataTransfer.getData('text/name'));
    ...
    private async _loadNodeDesignerState()
    {
        ...
        this.metadata.forEach(meta =>
        {
            ...
            {
                const component = this.nodeComponentPallete.availableComponents.find(itm => itm.component.ɵId == meta.id);
                const relationsMetadata = this.relationsMetadata.find(itm => itm.id == meta.id);

                this._setComponentAsUsed(component);

                const svgNode = this.nodeDesigner.addComponent(meta.position, component.component, component.metadata, relationsMetadata.nodeOptions);
            ...
            {
                const relationsMetadata = this.relationsMetadata.find(itm => itm.id == meta.id);
                const designerRelationsMetadata = this.nodeComponentPallete.nodesDefinitions.find(itm => itm.nodeType == relationsMetadata.nodeType);

                const svgNode = this.nodeDesigner.addNode(meta.position, designerRelationsMetadata, relationsMetadata.nodeOptions);
            ...
            if(relation.outputs && relation.outputs.length)
            {
                relation.outputs.forEach(output =>
                {
                    if(output.inputs && output.inputs.length)
                    {
                        ...
                        {
                            const svgRelation = relation.svgNode.addOutputRelation(output.outputName);
                            const inputPeer = relations.find(itm => itm.id == input.id);
                            const dynamic = !inputPeer.metadata.inputs || !inputPeer.metadata.inputs.find(itm => itm.id == input.inputName);
    ...
    {
        const index = this.nodeComponentPallete.availableComponents.indexOf(component);
    ...
    {
        const changes = this._iterableDiffer.diff(this._componentSvc.components);
        ...
        if(changes)
        {
            ...
            changes.forEachRemovedItem(removed =>
            {
                ...
                if(found)
                {
                    const index = this.nodeComponentPallete.availableComponents.indexOf(found);
                    this.nodeComponentPallete.availableComponents.splice(index, 1);
                }
            ...
            const addedComponents: DesignerLayoutPlaceholderComponent[] = [];
            ...
            {
                const metadata = await this._packageLoader.getComponentsMetadata(component.packageName, component.componentName);
