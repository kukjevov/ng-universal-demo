...
export class DesignerPageComponent implements OnInit, OnDestroy
{
    ...
    public async ngOnInit()
    {
        ...
        this._urlChangeSubscription = this._route.params.subscribe(async (params: {id: string}) =>
        {
            ...
            const tmp = await this._http.get<RemoteDesignerState>(`api/dynamic/state/${this.id}`)
    ...
    public async save()
    {
        ...
        const relationsMetadata = JSON.stringify(await this.ɵNodeDesigner.nodeDesigner.metadata);
        const nodeDesignerMetadata = JSON.stringify(this.ɵNodeDesigner.nodeDesigner.designerMetadata);
